...
export default function UsersPage({ searchParams }: UsersPageProps) {
  ...
  const [users, setUsers] = useState<User[]>([
    ...
    { id: "4", name: "Fatma Öztürk", email: "fatma@example.com", role: "antrenor", status: "inactive" }, // Inactive example
  ])
  ...
  const filteredUsers = users.filter(user => {
    ...
    const matchesSelectedRole = selectedRoles.length === 0 || selectedRoles.includes(user.role);
    const matchesSelectedStatus = selectedStatuses.length === 0 || selectedStatuses.includes(user.status);
    ...
    setUsers(prevUsers =>
      prevUsers.map(user => {
        if (user.id === userId) {
          ...
          toast.success(`${user.name} kullanıcısının durumu ${newStatus === "active" ? "Aktif" : "Devre Dışı"} olarak güncellendi`);
  ...
  const handleSendWarning = (message: string) => {
    ...
    toast.success(`${selectedUserForWarning.name} kullanıcısına uyarı gönderildi.`);
    console.log(`Uyarı gönderildi: Kullanıcı ID: ${selectedUserForWarning.id}, Mesaj: ${message}`);
  ...
  const handleStatusCheckedChange = (status: string) => (checked: CheckedState) => {
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="bireysel_kullanici" className="text-blue-800 hover:bg-blue-100">
                            ...
                            <SelectItem value="antrenor" className="text-yellow-800 hover:bg-yellow-100">
                            Antrenör
                          </SelectItem>
                          <SelectItem value="kulup_uyesi" className="text-purple-800 hover:bg-purple-100">
                            Kulüp Üyesi
                      ...
                      <Button 
                        ...
                        size="icon"
                          ...
                          <Switch
                            ...
                            title={user.status === "active" ? "Kullanıcıyı devre dışı bırak" : "Kullanıcıyı aktif et"}
                          />
                        </div>
                    </TableCell>
                  </TableRow>
